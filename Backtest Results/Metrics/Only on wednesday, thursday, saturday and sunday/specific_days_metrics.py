...
def calculate_backtest_metrics(df):
    ...
    # Filter for target days: Wednesday(2), Thursday(3), Saturday(5), Sunday(6)
    target_days = [2, 3, 5, 6]
    df = df[df['Date'].dt.dayofweek.isin(target_days)]
    ...
    # Adjusted annualization factor for 4 trading days a week (52 weeks * 4 days = 208 days)
    metrics['Sharpe Ratio'] = (daily_mean / daily_std) * np.sqrt(208) if daily_std != 0 else 0
    ...
    if is_drawdown.any():
        # This still correctly calculates calendar days because it takes the max date - min date
...
def plot_performance_dashboard(df):
    ...
    fig.suptitle('Strategy Performance Dashboard (Wed, Thu, Sat, Sun Only)', fontsize=16)
...
if __name__ == "__main__":
    # Load data
    data_path = r"Strategies Github/Short Strangle/Backtest/After commisions:fees results/after_fees_Backtest_2025-01-01_to_2026-01-31.xlsx"
